...
class FileRecvHandler extends Thread 
{
	...
	public FileRecvHandler( long size, String projNumStr, String fileName ) 
	{
		// De transfer queue
	...
	public static String getClientText() {
		...
		return FileRecvHandler.clientText;
	}
	...
	{		
		...
		msg = MessageHandler.prepareMessage( RenderAPI.NetworkMessageType.DOWNLOADFILE, project, size, file, this.serverSock.getLocalPort() );
		
		// RenderCommands.doCmdSendFile() leest welke wait er is.
		FileRecvHandler.clientText = msg ;
		...
		for(TransferAttributes attributes: transferQueue ) {
			...
			message = transferObj.recvFile( sock, serverSock);
